...
def generate_ai_analysis(client, prompt):
    ...
    for model_name in models_queue:
        # 每個模型先嘗試啟用 Google Search grounding；
        # 免費方案的 grounding 配額極低（常見為 0），一旦遇到 429 就自動關閉 grounding 改用一般模式繼續嘗試
        use_grounding = True
        for attempt in range(3):
            try:
                current_config = search_config if use_grounding else plain_config
                ...
                if response.text:
                    ...
                    grounding_tag = "" if use_grounding else "，未啟用搜尋"
                    version_tag = f"{model_name} [備用模型{grounding_tag}]" if is_backup else f"{model_name}{grounding_tag}"
                ...
                # 若遇到 429 且目前仍啟用 grounding，優先懷疑是「搜尋配額用盡」（免費方案常見為 0），
                # 立即關閉 grounding 改用一般模式重試，不做等待退避（等待對配額用盡的情況沒有幫助）
                if "429" in err_msg and use_grounding:
                    print(f"[{model_name}] 疑似 Google Search grounding 配額用盡，改用一般模式重試")
                    use_grounding = False
                    continue

                # 若遇到 503/429（非 grounding 造成）流量限流，退避等待後重試
